...
super <- function(keyword) {
    ...
    if(length(ict)) res = c(res, semrel[[3]][1, ict])
    res
}
...
sib <- function(keyword) {
    ...
    if(length(ire2)) res = c(res, semrel[[1]][1, ire2])
    res
}
...
conn.vector <- function(rel, keyword, is_super=FALSE, is_sib=FALSE) {
    ...
    iv = which(iv > 0)
    if(is_super) {
        leave = which(iv == super(ik))
        iv = iv[leave]
        v = v[leave]
    }
    if(is_sib) {
        leave = which(iv == sib(ik))
        iv = iv[leave]
        v = v[leave]
    }
    bv = rep(0, nkeywords())
...
main.cooccur <- function(k, n, index=FALSE) {
    ik <- keyword.index(k)
    ifelse(index,
        inputm[1:n, sort(c(2*rel-1 + 2*rn*(ik-1), 2*rel + 2*rn*(ik-1)))],
        inputm[1:n, 2*rel + 2*rn*(ik-1)])
}
...
merge.keywords <- function(cluster) {
    ...
    for(r in 1:rn) {
        ...
        v = v[ind][1:m]
        iv = iv[ind][1:m]
        if(ncol(inputm) < 2*r-1 + 2*rn*(newk-1))
            inputm <<- cbind(inputm, matrix(0, nrow=m, ncol=2*rn))
        inputm[, 2*r-1 + 2*rn*(newk-1)] <<- iv
        inputm[, 2*r + 2*rn*(newk-1)] <<- v
    }
    reldb_df[[index]] <<- rel
    names(relbd_df)[index] <<- newk
